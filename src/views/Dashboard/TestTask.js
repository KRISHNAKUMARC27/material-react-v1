...
const iconsMapping = {
  "View results": "auto_awesome_motion",
  "Rerun Test Task": "replay",
  "Perform Comparison Run": "compare",
  "Delete Test Task": "delete",
  "Stop Test Task": "stop",
};
...
const statusMapping = {
  ...
  INPROGRESS: {
    color: "warning",
    actions: ["View results", "Stop Test Task", "Rerun Test Task"],
  },
  STOPPED: {
    color: "danger",
    actions: ["View results", "Rerun Test Task", "Delete Test Task"],
  },
};
...
export default function TestTask() {
  ...
  const downloadDDRResponse = (txnId) => {
    fetch(process.env.REACT_APP_API_URL + "/dataDog/response/" + txnId)
      .then(async (response) => {
        if (!response.ok) {
          const errorText = await response.text();
          throw new Error(errorText || response.statusText);
      ...
      .then((data) => {
        ...
        const fileName = "DotDataResponse_" + txnId + ".json";
        saveAs(blob, fileName);
        setSnackMessage("Downloading " + fileName);
        setSnackColor("success");
        setSnackOpen(true);
      })
      .catch((err) => {
        setSnackMessage("Error while extracting DotDataResponse for " + txnId);
        setSnackColor("error");
        setSnackOpen(true);
        console.log(err.message);
      });
  ...
  const handleNewComptplanClickOpen = (task) => {
  ...
  React.useEffect(() => {
    let intervalId = null;
    if (newtreportopen && tTask.status === "INPROGRESS") {
      intervalId = setInterval(() => {
        handleNewtreportClickOpen(tTask);
        console.log("Reloading open testTask");
      }, 10000);
    }
  ...
  }, [newtreportopen, tTask.status]);

  const handleNewtreportClickOpen = (task) => {
    fetch(process.env.REACT_APP_API_URL + "/testTask/" + task.id)
      .then(async (response) => {
        if (!response.ok) {
          const errorText = await response.text();
          throw new Error(errorText || response.statusText);
      ...
      .then((data) => {
        ...
        fetch(process.env.REACT_APP_API_URL + "/testTask/testCase/" + task.id)
          .then(async (response) => {
            if (!response.ok) {
              const errorText = await response.text();
              throw new Error(errorText || response.statusText);
          ...
          .catch((err) => {
            console.log(err.message);
            setSnackMessage("Error in fetching TestCase status " + err.message);
            setSnackColor("error");
            setSnackOpen(true);
          });
      })
      .catch((err) => {
        console.log(err.message);
        setSnackMessage("Error in fetching TestTask status " + err.message);
        setSnackColor("error");
        setSnackOpen(true);
      });
  };
  ...
  const findWithFilters = async (payload) => {
    await fetch(process.env.REACT_APP_API_URL + "/testTask/findWithFilters", {
      method: "POST",
      body: JSON.stringify(payload),
      headers: {
        "Content-type": "application/json; charset=UTF-8",
      ...
      <Card>
        <CardHeader color={color} stats icon>
          ...
          <p className={classes.cardCategory}>
            {name} <br></br> {suiteName}
          </p>
          <h6 className={classes.cardTitle}>{status}</h6>
        </CardHeader>
        <CardFooter stats>
          <div className={classes.stats}>
            {actions.map((action, index) => (
              ...
              >
                <ColoredWrapper color={color}>
                  <Tooltip title={action}>
                    <Icon>{iconsMapping[action]}</Icon>
                  </Tooltip>
                </ColoredWrapper>
              </a>
            ))}
  ...
  const SelectField = ({ id, label, value, handleChange, options }) => (
    <FormControl className={classes.formControl} variant="outlined">
      <InputLabel id={`${id}-label`} shrink={value.length > 0}>
        {label}
      </InputLabel>
      <Select
        labelId={`${id}-label`}
          ...
          (option) =>
            option && (
              <ListItem
                ...
                selected={value.includes(option.name || option)}
                style={
                  value.includes(option.name || option)
  ...
  return (
    <div>
      <Grid container spacing={3}>
        ...
        <Grid item xs={2}>
          <SelectField
            ...
            label="EnvType"
            value={envType}
            handleChange={handleEnvTypeChange}
            options={Environment}
          />
        </Grid>
        <Grid item xs={2}>
        ...
        <Grid item xs={2}>
          <SelectField
            ...
            label="TestSuite"
            value={suiteName}
            handleChange={handleSuiteNameChange}
            options={ConfigTestSuite.map((option) => ({
              id: option.id,
              name: option.name,
            }))}
          />
      ...
      <GridContainer>
        {testTasks.map((task) => {
          const taskStatus =
            task.status === "COMPLETED" && task.baseline
              ? "COMPLETED_BASELINE"
              : task.status;
          const statusData = statusMapping[taskStatus] || {};
          ...
          return (
            <GridItem key={task.id} xs={12} sm={6} md={3}>
              <TestTaskCard
                ...
                id={task}
                onClicks={{
                  "View results": handleNewtreportClickOpen,
                  "Rerun Test Task": rerunTestTask,
                  // "Perform Comparison Run": setNewComptplanopenFeatureVM,
                  "Perform Comparison Run": handleNewComptplanClickOpen,
                  "Delete Test Task": deleteTestTask,
                  "Stop Test Task": stopTestTask,
                }}
              />
            </GridItem>
          );
